 ...
 *    -f: mtry factor - mtry is usually square root of # snps in window, this multiplies that number
...
ProcessedInput* processInput(int argc, char ** argv, ofstream * logfile){
  ...
  istringstream mtryStringConverter(params["-f"]);
  if(!(mtryStringConverter >> mtryFactor)){
    cout << "Invalid mtry Factor" << endl;
    exit(1);
  }
  ...
  if(params["-x"].length() != 0){
    ...
    while(!excludes.eof()){
      ...
      if(excludeIndices.size() > 0 && excludeIndices.back() > excludeIndex){
        cout << "Exclude indices not in order " << excludeIndices.back() << " " << excludeIndex<< endl;
        exit(1);
      }
  ...
  int numWindows = windowBeginLocs->size();
  ...
  int ** ancWindows = new int*[numWindows];
  for(int windex = 0; windex < numWindows; windex++){
    ...
    ancWindows[windex] = new int[windowBlockSize];
  }
  
  // EM: nothing changed because changed numAdm above
  int ** admWindows = new int*[numWindows];
  for(int windex = 0; windex < numWindows; windex++){
    int numSNPsInWindow = (*windowEndIndexes)[windex] - (*windowBeginIndexes)[windex] + 1;
    int windowBlockSize = numSNPsInWindow * numAdm;
    admWindows[windex] = new int[windowBlockSize];
  }
  ...
  // Get number of haps in each ref class for stratified bootstrap sampling
  // EM : Affects: numHapsInEachPop and popHapLocations, moved to randomforest
  
  
  //Step 9: Store the data
  ProcessedInput* processedInput = new ProcessedInput();
  processedInput->numSNPs = numSNPs;
  processedInput->numWindows = numWindows;
  ...
  processedInput->mtryFactor = mtryFactor;
