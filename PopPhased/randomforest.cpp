...
void Tree(int numHomogSNPs, bool* snpHomogParam, int windex, int firstSNPIndex, int windowNumSNPs, unsigned int mtry, int numAncPops, int bootstrapSample[], int numAdm, int numAnc, int* ancHapClasses, int ** ancWindows, int ** admWindows, int admHere[], double** treeCounts, int depth);
...
double ** RandomForest(ProcessedInput* processedInput, double** admClassProbs, int emIteration){
  ...
  int ** ancWindows = processedInput->ancWindows;
  //int ** admWindows = processedInput->admWindows;
  int ** admWindowsPhasings = processedInput->admWindowsPhasings;
...
//          }
          ...
          Tree(0, snpHomog, windex, firstSNPIndex, windowNumSNPs, mtry, numAncPops, bootstrapSample, thisWindowNumAdm, numAnc, ancHapClasses, ancWindows, admWindowsPhasings, admHere, treeCounts, 0);
        }
...
void Tree(int numHomogSNPs, bool* snpHomogParam, int windex, int firstSNPIndex, int windowNumSNPs, unsigned int mtry, int numAncPops, int bootstrapSample[], int numAdm, int numAnc, int * ancHapClasses, int ** ancWindows, int ** admWindows, int admHere[], double** treeCounts, int depth){
  ...
  //  }
    ...
    while(indexOfMinGini == NOGOOD){
      ...
      double minGini = 1.0;
      int classCounts0[numAncPops];
      int classCounts1[numAncPops];
    ...
    int bootstrapSample0[numAnc];
    int bootstrapSample1[numAnc];
    for(int ancIndex = 0; ancIndex < numAnc; ancIndex++){
    ...
    if(go0){
      //      cout << "Going 0\n";
      Tree(numHomogSNPs, snpHomog, windex, firstSNPIndex, windowNumSNPs, mtry, numAncPops, bootstrapSample0, numAdm, numAnc, ancHapClasses, ancWindows, admWindows, admHere0, treeCounts, depth+1);
    ...
    if(go1){
      //      cout << "Going 1\n";
      Tree(numHomogSNPs, snpHomog, windex, firstSNPIndex, windowNumSNPs, mtry, numAncPops, bootstrapSample1, numAdm, numAnc, ancHapClasses, ancWindows, admWindows, admHere1, treeCounts, depth+1);
